...
private fun <T, U, V> appender(
  tPath: PathTemplate<T>,
  uPath: PathTemplate<U>,
  fromParts: (Tuple2<T, U>) -> V,
  toParts: (V) -> Tuple2<T, U>
) =
...
operator fun <T1, T2, T3, T4, T5> PathTemplate<Tuple2<T1, T2>>.div(rest: PathTemplate<Tuple3<T3, T4, T5>>) =
  appender(
    this,
    rest,
    { tupleFlat(it.val1, it.val2) },
    { tuple(tuple(it.val1, it.val2), tuple(it.val3, it.val4, it.val5)) }
  )
...
operator fun <T1, T2, T3, T4, T5> PathTemplate<Tuple3<T1, T2, T3>>.div(rest: PathTemplate<Tuple2<T4, T5>>) =
  appender(
    ...
    rest,
    { tupleFlat(it.val1, it.val2) },
    { tuple(tuple(it.val1, it.val2, it.val3), tuple(it.val4, it.val5)) }
  )
